...
CmdLineParser::Option g_options[] =
{
   ...
   {SEPARATOR_OPER, '\0', "",              CmdLineParser::E_NO_ARG,  0, "OPERATIONS:\r\n"},
   ...
   {SEPARATOR_ALL,  '\0', "",              CmdLineParser::E_NO_ARG,  0, "The following apply to RUNNABLE, CONDITION, and THREADBARRIER\r\n"},
   {NOBLOCK_OPT,     'x', "noblock",       CmdLineParser::E_NO_ARG,        0, "Don't block when adding work to queue.  (Default = block)  With threadPools, you can add work in two ways:  tryAddWork, which if the queue is full it fails, or addWork, which if the queue is full, it blocks until there is space to add the work.  This allows for demonstrating both ways"},
   ...
   {SEPARATOR_RUNNABLE,'\0', "",           CmdLineParser::E_NO_ARG,  0, "RUNNABLE options:\r\n"},
   {THREADCOUNT_OPT, 'n', "totalThreads",  CmdLineParser::E_OPTIONAL_ARG,  0, "Total number of threads to use (put into the threadpool queue, threads to create, etc).  Default=12"},
   {NUMTHREADS_OPT,  'a', "activeThreads", CmdLineParser::E_OPTIONAL_ARG,  0, "ThreadPool Threads to use at one time.  Default=5"},
   {QUEUE_OPT,       'q', "queueSize",     CmdLineParser::E_OPTIONAL_ARG,  0, "ThreadPool Queue size.  Default = 8.  Must be > number of Active Threads"},
   {POOLTYPE_OPT,    'p', "poolType",      CmdLineParser::E_OPTIONAL_ARG,  0, "ThreadPool Type (0, 1, or 2).  0=Fixed, 1=Dynamic (Default),  2=DynamicNoQueue"},
   {LOOP_OPT,        'l', "loop",          CmdLineParser::E_OPTIONAL_ARG,  0, "How many times each thread should loop before it is quits (Default = 10)"},
   {INTERVAL_OPT,    'i', "interval",      CmdLineParser::E_OPTIONAL_ARG,  0, "Interval (in seconds) between creation of each thread.  Default = 1."},
   {LOOP_WAIT_OPT,   'w', "loopwait",      CmdLineParser::E_OPTIONAL_ARG,  0, "How long to wait (in seconds) between each loop in the thread.  Default = 1.\r\n"},
   {SEPARATOR_CONDITION,'\0', "",          CmdLineParser::E_NO_ARG,  0, "CONDITION options:\r\n"},
